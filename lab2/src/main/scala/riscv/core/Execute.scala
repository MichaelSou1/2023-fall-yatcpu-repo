...
io.csr_reg_write_data := MuxLookup(funct3, 0.U)(IndexedSeq(
  ...
  InstructionsTypeCSR.csrrc -> (io.csr_reg_read_data & (~io.reg1_data)),  // CSRRC instruction
  InstructionsTypeCSR.csrrwi -> (io.instruction(19, 15)),
  InstructionsTypeCSR.csrrsi -> (io.csr_reg_read_data | (io.instruction(19, 15))),
  InstructionsTypeCSR.csrrci -> (io.csr_reg_read_data & (~Cat(0.U(27.W), io.instruction(19, 15))))
))
