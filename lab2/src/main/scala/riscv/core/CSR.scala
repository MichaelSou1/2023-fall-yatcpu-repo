...
class CSR extends Module {
  ...
  //lab2(CLINTCSR)
//what data should be passed from csr to clint
io.clint_access_bundle.mstatus := Mux(io.reg_write_enable_id && io.reg_write_address_id === CSRRegister.MSTATUS, 
  io.reg_write_data_ex, 
  mstatus)
io.clint_access_bundle.mtvec := mtvec
io.clint_access_bundle.mcause := Mux(io.reg_write_enable_id && io.reg_write_address_id === CSRRegister.MCAUSE, 
  io.reg_write_data_ex, 
  mcause)
io.clint_access_bundle.mepc := Mux(io.reg_write_enable_id && io.reg_write_address_id === CSRRegister.MEPC, 
  io.reg_write_data_ex, 
  mepc)
